...
async function updateSchedule(
    ...
    newInfo: { start: string, end: string, start_time: string, end_time: string, repeat: string, grade: string, gradeClass: string, obs?: string, speaker?: string, lab?: string, chromes?: string[], weekdays?: string[] }
) {
    try {
        const schedule_info = await pb.collection(`${type}_schedule_info`).update(id, {
            grade: newInfo.grade,
    ...
    if (info.start != newInfo.start || info.end != newInfo.end || info.start_time != newInfo.start_time || info.end_time != newInfo.end_time || 
        info.repeat !== newInfo.repeat || info.weekdays != newInfo.weekdays || info.chromes != newInfo.chromes) {
        const toDelete = schedules.filter(s => s.schedule_info == id);
        if (toDelete.length > 0) {
            const deleteSchedules = pb.createBatch();
            for (let i = 0; i < toDelete.length; i++) {
                deleteSchedules.collection(`${type}_schedule`).delete(toDelete[i].id);
            }

            try {
                const deleteResult = await deleteSchedules.send();
            } catch (error) {
                console.log('Error deleting old schedules!', error);
            }
        }
        ...
        scheduledDays.forEach(range => {
            if (type == services.chrome) batch.collection('chrome_schedule').create({
                schedule_info: id,
                start: range.start,
                end: range.end,
                chrome: newInfo.chromes,
            ...
            if (type == services.lab) batch.collection('lab_schedule').create({
                schedule_info: id,
                start: range.start,
                end: range.end,
                lab: newInfo.lab,
            ...
            if (type == services.speaker) batch.collection('speaker_schedule').create({
                schedule_info: id,
                start: range.start,
                end: range.end,
                speaker: newInfo.speaker,
        ...
        try {
            const newSchedules = await batch.send();
        } catch (error) {
            console.log('Error creating new schedules!', error);
        }
    }
