...
export default function Schedules({ data }: { data: any }) {
    ...
    // Calcula quantidades por status
                        ...
                        {data.chromeSchedule.map((schedule: RecordModel) => {
                                if (format(new Date(Date.parse(schedule.start) + 10800000), 'dd/MM/yyyy') == format(new Date(new Date()), 'dd/MM/yyyy') && !schedule.returned) return (
                                    <div key={schedule.id} className={`${styles.schedule}`}>
                                        <div className={`${styles.header}`}>
                                            <h3>{`${schedule.expand?.schedule_info.expand?.user.name}${schedule.expand?.schedule_info.obs ? `: "${schedule.expand?.schedule_info.obs}"` : ''} | ${schedule.expand?.schedule_info.expand?.grade.label + (schedule.expand?.schedule_info.expand?.class ? ` ${schedule.expand?.schedule_info.expand?.class.label}` : '')}, das ${schedule.expand?.schedule_info.start_time} até ${schedule.expand?.schedule_info.end_time}`}</h3>
                                        </div>
                                ...
                                if (schedule.late && !schedule.returned) return (
                                    <div key={schedule.id} className={`${styles.schedule}`}>
                                        <div className={`${styles.header} ${styles.late}`}>
                                            <h3>{`${schedule.expand?.schedule_info.expand?.user.name}${schedule.expand?.schedule_info.obs ? `: "${schedule.expand?.schedule_info.obs}"` : ''} | ${schedule.expand?.schedule_info.expand?.grade.label + (schedule.expand?.schedule_info.expand?.class ? ` ${schedule.expand?.schedule_info.expand?.class.label}` : '')}, em atraso desde ${format(new Date(Date.parse(schedule.end)), 'dd/MM/yyyy')}!`}</h3>
                                        </div>
